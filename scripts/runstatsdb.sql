...
create table perf_measurements(stepid integer, run_id integer, step_time integer, input_files text, output_files text, start_time integer, end_time integer, run_time integer, step_hash text, reference_file text, step_body text, foreign key(stepid) references perf_steps(id), foreign key(run_id) references run_timing_metadata(rowid));
